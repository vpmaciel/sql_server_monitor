...
class EmpresaRecuperaSenhaController extends CI_Controller
{

    public function VerRegistro()
    {
        $this->load->view('EmpresaRecuperaSenhaView');
    }
    ...
    public function RecuperarSenha()
    {
        ...
        if (! $this->form_validation->run()) {
            $this->load->view('EmpresaRecuperaSenhaView', $regras);
            return;
        }
        $email = $this->input->post('email');
        if ($email) {
            $cadastros = $this->EmpresaModel->ObterRegistro(NULL,$email);
